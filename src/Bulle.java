...
public class Bulle extends Entity {
    ...
    public static void groupBulles (Bulle[][] bulles, double widthScreen,
        ...
        for (int i = 0; i < bulles.length; i++) {
            ...
            // générer aléatoirement une coordonnée baseX
            baseX[i] = random.nextDouble() * (widthScreen + 1);

            for (int j = 0; j < bulles[0].length; j++) {

                // calculer rayon pour chaque bulle
                double rayon = random.nextDouble() *
                        (maxRayon + 1 - minRayon) + minRayon;

                // calculer vitesse pour chaque bulle
                double vitesseY = random.nextDouble() *
                        (Bulle.vitesseMax + 1 - Bulle.vitesseMin) +
                        Bulle.vitesseMin;

                // calculer la position initiale X pour chaque bulle
                double base = baseX[i];
                double initX = random.nextDouble() *
                        (base + borne + 1 - base + borne) + base - borne;
